...
const CALIBRATE_BODY_MAP = false  // set false after calibration is complete
const OC_REGION_COORDS = {
  "Head": { f: [49.8, 14.5], b: [53, 13.8] },
  "Neck": { f: [49.8, 21], b: [53, 18.9] },
  "Shoulder L": { f: [67.4, 25.6], b: [75, 24.4] },
  "Shoulder R": { f: [30, 25.2], b: [32.6, 24.4] },
  "Upper Arm L": { f: [71.3, 31.5], b: [76.7, 30.9] },
  "Upper Arm R": { f: [25.6, 31.1], b: [28.8, 30.5] },
  "Elbow L": { f: [72.9, 35.9], b: [80, 36.6] },
  "Elbow R": { f: [22.3, 36.2], b: [27.1, 37] },
  "Forearm L": { f: [77.3, 43.1], b: [83.3, 41.6] },
  "Forearm R": { f: [18.4, 42.7], b: [22.7, 41.6] },
  "Wrist L": { f: [79, 46.5], b: [84.9, 46] },
  "Wrist R": { f: [17.3, 47.1], b: [19.9, 45.6] },
  "Chest": { f: [48.7, 30.9], b: null },
  "Upper Back": { f: null, b: [53.5, 25] },
  "Lower Back": { f: null, b: [53, 37.2] },
  "Core/Abs": { f: [48.1, 39.1], b: null },
  "Hip L": { f: [63.6, 43.7], b: [68.9, 41.8] },
  "Hip R": { f: [32.7, 45.2], b: [34.3, 42.5] },
  "Glute L": { f: null, b: [63.4, 46.3] },
  "Glute R": { f: null, b: [42, 46.5] },
  "Quad L": { f: [60.8, 52.6], b: null },
  "Quad R": { f: [34.9, 52.2], b: null },
  "Hamstring L": { f: null, b: [65.1, 54] },
  "Hamstring R": { f: null, b: [41.4, 53.4] },
  "IT Band L": { f: [55.3, 58.9], b: [64, 57.8] },
  "IT Band R": { f: [40.4, 58], b: [41.4, 58.7] },
  "Knee L": { f: [59.7, 62.1], b: [64, 62.5] },
  "Knee R": { f: [34.9, 61.8], b: [39.8, 63.1] },
  "Shin L": { f: [59.1, 71.1], b: null },
  "Shin R": { f: [35.5, 71.7], b: null },
  "Calf L": { f: null, b: [67.3, 69.4] },
  "Calf R": { f: null, b: [37.6, 69] },
  "Ankle L": { f: [58.6, 77.8], b: [64, 79.5] },
  "Ankle R": { f: [34.9, 78.1], b: [39.8, 79.1] },
  "Toe L": { f: [61.4, 85.2], b: [35.4, 82.5] },
    ...
    try {
      ...
      if (biometricResult.length) {
        ...
        if (setBiometricRecords) setBiometricRecords(prev => {
          ...
          try { localStorage.setItem("lift_biometric_records", JSON.stringify(out)); } catch {}
          return out
        })
  ...
  fetch("/data/weight_daily.json")
    ...
    .then(fetched => {
      ...
      setBiometricRecords(prev => {
        ...
        console.log(`[biometricRecords/weight_daily.json] prev=${Array.isArray(prev) ? prev.length : 0} fetched=${fetched.length} → ${merged.length} rows`)
        return merged;
      });
    })
    .catch(() => {});
        ...
        fetch(`${base}data/weight_daily.json`)
          ...
          .then(weightSeed => {
            ...
            setBiometricRecords(prev => {
              ...
              const seeded = weightSeed.filter(r => r.measured_date && r.weight_lb && Number(r.weight_lb) > 140)
              console.log(`[biometricRecords/weight_daily-seed] prev=0 seed=${seeded.length} rows`)
              return seeded
            })
          })
          .catch(() => {})
  ...
  ;(async () => {
    try {
      ...
      if (remoteBiometricRecords.length) {
        setBiometricRecords(prev => {
          // Merge key: measured_date (covers weight_daily.json entries that only have measured_date)
          const dateKey = r => String(r.measured_date || r.date || r.timestamp || "").slice(0, 10)
          const hasWeight = r => r.weight_lb != null && Number.isFinite(Number(r.weight_lb)) && Number(r.weight_lb) > 0
          const byDate = {}
          // Seed from prev — preserves Excel/weight_daily entries
          ;(Array.isArray(prev) ? prev : []).forEach(r => {
            const d = dateKey(r)
            if (!d) return
            const cur = byDate[d]
            if (!cur || (!hasWeight(cur) && hasWeight(r))) byDate[d] = r
          })
          // Merge Supabase remote rows — never shrink what prev already has
          remoteBiometricRecords.forEach(r => {
            const d = dateKey(r)
            if (!d) return
            const cur = byDate[d]
            if (!cur) { byDate[d] = r; return }
            // Trainer entries are authoritative — never overwrite with non-trainer
            if (cur.source === "trainer" && r.source !== "trainer") return
